...
fGDB = r"C:\LPA\Projects\CursorProject\CursorProject.gdb"
tableName = "TestTable"
fgdbTable = rf"{fGDB}\{tableName}"
...
fidDataDict = {
    row[0]: [row[n] for n in range(1, len(fieldList) + 1)]
    for row in arcpy.da.SearchCursor(shp, fidFieldList, "FID < 25")
}
print(fidDataDict)
...
for i in range(0, len(fieldList)):
    for j in range(0, len(fidDataDict)):
        ...
        tableCellGeom = arcpy.Polygon(
            arcpy.Array(
                [
                    arcpy.Point(accumWidth, cellHeight * j),
                    arcpy.Point(accumWidth, (cellHeight * j) + cellHeight),
                    arcpy.Point(
                        accumWidth + fieldLengthList[i], (cellHeight * j) + cellHeight
                    ),
